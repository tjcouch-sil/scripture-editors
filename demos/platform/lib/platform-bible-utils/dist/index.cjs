...
var Sn = (t, e, r) =>
  e in t ? wn(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : (t[e] = r);
var z = (t, e, r) => Sn(t, typeof e != "symbol" ? e + "" : e, r);
Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
const On = require("async-mutex"),
  or = require("jsonpath-plus"),
  Rt = class Rt {
    constructor(e, r = 1e4) {
      z(this, "variableName");
      ...
      ((this.variableName = e),
        ...
        (this.promiseToValue = new Promise((u, o) => {
    ...
    resolveToValue(e, r = !1) {
      if (this.resolver)
        (Rt.verboseLoggingEnabled && console.debug(`${this.variableName} is being resolved now`),
          this.resolver(e),
          this.complete());
      else {
    ...
    rejectWithReason(e, r = !1) {
      if (this.rejecter)
        (Rt.verboseLoggingEnabled &&
          console.debug(`${this.variableName} is being rejected now with reason: ${e}`),
          this.rejecter(e),
          this.complete());
      else {
        if (r) throw Error(`${this.variableName} was already settled`);
...
function Fn(t) {
  ...
  return Ie(t) ? t.includes(e) : Kt(t).includes(e);
}
function qn(t) {
  const e = "401 Unauthorized error while getting shared projects.",
    r = "User registration is not valid. Cannot retrieve resources from DBL.",
    u = Ie(t) ? t : Kt(t);
...
class Lr {
  constructor(e, r) {
    z(this, "baseDocument");
    z(this, "contributions", new Map());
    z(this, "latestOutput");
    z(this, "options");
    z(this, "onDidRebuildEmitter", new _r());
  ...
  updateBaseDocument(e) {
    return (
      this.validateBaseDocument(e),
      (this.baseDocument = this.options.copyDocuments ? Et(e) : e),
  ...
  addOrUpdateContribution(e, r) {
    ...
    let o = this.options.copyDocuments && r ? Et(r) : r;
    ((o = this.transformContributionAfterValidation(e, o)), this.contributions.set(e, o));
    try {
      return this.rebuild();
    } catch (a) {
      throw (
        u ? this.contributions.set(e, u) : this.contributions.delete(e),
        new Error(`Error when setting the document named ${e}: ${a}`)
      );
    }
  }
  deleteContribution(e) {
    ...
    try {
      ...
      throw (
        this.contributions.set(e, r),
        new Error(`Error when deleting the document named ${e}: ${u}`)
      );
    }
  }
  deleteAllContributions() {
    ...
    try {
      ...
      throw (
        e.forEach(([u, o]) => this.contributions.set(u, o)),
  ...
  rebuild() {
    if (this.contributions.size === 0) {
      let r = Et(this.baseDocument);
      return (
        (r = this.transformFinalOutputBeforeValidation(r)),
        this.validateOutput(r),
        (this.latestOutput = r),
        this.onDidRebuildEmitter.emit(void 0),
        this.latestOutput
      );
    }
    let e = this.baseDocument;
    return (
      this.contributions.forEach((r) => {
        ((e = $n(e, r, this.options.ignoreDuplicateProperties)), this.validateOutput(e));
      }),
      (e = this.transformFinalOutputBeforeValidation(e)),
  ...
  if (ar(t, e)) {
    ...
    Object.keys(o).forEach((a) => {
      if (Object.hasOwn(u, a)) {
        if (ar(u[a], o[a])) u[a] = kr(u[a], o[a], r);
        else if (sr(u[a], o[a])) u[a] = u[a].concat(o[a]);
        else if (!r)
          throw new Error(`Cannot merge objects: key "${a}" already exists in the target object`);
      } else u[a] = o[a];
    });
  } else sr(t, e) && t.push(...e);
  return t;
}
class jn {
  constructor(e) {
    ...
    ((this.bufferSize = e),
      (this.ringBuffer = new Array(e).fill(0)),
      (this.writerIndex = 0),
      (this.readerIndex = 0),
  ...
  recordInstance() {
    ...
    ((this.lastTimeDifference = e - this.ringBuffer[this.readerIndex]),
      (this.ringBuffer[this.writerIndex] = e),
      (this.writerIndex += 1),
      this.writerIndex >= this.bufferSize && (this.writerIndex %= this.bufferSize),
      this.writerIndex === this.readerIndex &&
        ((this.readerIndex += 1),
        this.readerIndex >= this.bufferSize && (this.readerIndex %= this.bufferSize)));
  }
  hasViolatedThreshold(e) {
    return this.totalInstanceCount >= this.bufferSize && this.lastTimeDifference < e;
  }
}
class Fr extends On.Mutex {}
class Un {
  constructor() {
    z(this, "mutexesByID", new Map());
  }
  get(e) {
    let r = this.mutexesByID.get(e);
    return r || ((r = new Fr()), this.mutexesByID.set(e, r), r);
  }
}
class Vn extends Lr {
  constructor(e, r) {
...
class Gn {
  ...
  cleanupPromiseChain(e) {
    ...
    const u = { promise: zn },
      o = r
        .catch((a) => this.logger.warn(`Error in promise for ${e}: ${a.message}`))
        .finally(() => {
...
class $r {
  ...
  findClosestLessThanOrEqual(e) {
    ...
    if (r === -1) return;
    const u = this.sortedKeys[r],
      o = this.map.get(u);
    if (o !== void 0) return { key: u, value: o };
  }
  binarySearchLessThanOrEqual(e) {
    let r = 0,
      u = this.sortedKeys.length - 1,
      o = -1;
    for (; r <= u; ) {
...
class Hn {
  ...
  insert(e) {
    ...
    return r < this.items.length && this.compareFn(this.items[r], e) === 0
      ? !1
      : (this.items.splice(r, 0, e), !0);
  }
  remove(e) {
  ...
  at(e) {
    if (!(e < 0 || e >= this.items.length)) return this.items[e];
  }
  forEach(e) {
  ...
  [Symbol.iterator]() {
    ...
    return {
      next: () =>
        e < this.items.length
          ? ((e += 1), { value: this.items[e - 1], done: !1 })
          : { value: void 0, done: !0 },
    };
  }
  clear() {
...
class Jn {
  ...
  async runAllUnsubscribers() {
    const e = [...this.unsubscribers].map((u) => u()),
    ...
    return (
      ...
      r.every(
        (u, o) => (
          u ||
            console.error(`UnsubscriberAsyncList ${this.name}: Unsubscriber at index ${o} failed!`),
          u
        ),
      )
    );
...
function Xn(t) {
  ...
  if (Ie(t)) return { message: t, platformErrorVersion: Ot };
  if (typeof t == "object" && "message" in t && typeof t.message == "string") {
    const e = { message: t.message, platformErrorVersion: Ot };
    return (
      Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)),
      Object.defineProperty(e, "message", { enumerable: !0 }),
      "stack" in t &&
        Ie(t.stack) &&
        Object.defineProperty(e, "stack", { value: t.stack, enumerable: !0 }),
      "cause" in e && Object.defineProperty(e, "cause", { enumerable: !0 }),
      e
    );
  }
...
var Kn = Object.defineProperty,
  Wn = (t, e, r) =>
    e in t ? Kn(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : (t[e] = r),
  H = (t, e, r) => Wn(t, typeof e != "symbol" ? e + "" : e, r);
const at = [
    "GEN",
    "EXO",
    "LEV",
  ...
  cr = au();
function vt(t, e = !0) {
  return (e && (t = t.toUpperCase()), t in cr ? cr[t] : 0);
}
function Qt(t) {
  return vt(t) > 0;
}
...
function Zt(t, e = "***") {
  const r = t - 1;
  return r < 0 || r >= at.length ? e : at[r];
}
function zr(t) {
  return t <= 0 || t > Vr ? "******" : jr[t - 1];
}
function uu(t) {
  return zr(vt(t));
}
...
function au() {
  const t = {};
  for (let e = 0; e < at.length; e++) t[at[e]] = e + 1;
  return t;
}
const fe = {
  allBookIds: at,
...
const Te = class {
  constructor(e) {
    if (
      ...
      H(this, "fullPath"),
      H(this, "isPresent"),
      H(this, "hasVerseSegments"),
      H(this, "isCustomized"),
      H(this, "baseVersification"),
      H(this, "scriptureBooks"),
      H(this, "_type"),
      e == null)
    )
      throw new Error("Argument undefined");
    typeof e == "string"
      ? ((this.name = e), (this._type = Le[e]))
      : ((this._type = e), (this.name = Le[e]));
  }
  get type() {
    return this._type;
  }
...
(H(Te, "Original", new Te(Le.Original)),
  H(Te, "Septuagint", new Te(Le.Septuagint)),
  H(Te, "Vulgate", new Te(Le.Vulgate)),
  H(Te, "English", new Te(Le.English)),
  H(Te, "RussianProtestant", new Te(Le.RussianProtestant)),
  H(Te, "RussianOrthodox", new Te(Le.RussianOrthodox)));
let ut = Te;
function lr(t, e) {
  const r = e[0];
  for (let u = 1; u < e.length; u++) t = t.split(e[u]).join(r);
  return t.split(r);
}
var Hr = ((t) => (
  (t[(t.Valid = 0)] = "Valid"),
...
const ve = class K {
  constructor(e, r, u, o) {
    if (
      ...
      H(this, "lastChapter"),
      H(this, "lastVerse"),
      H(this, "hasSegmentsDefined"),
      H(this, "text"),
      H(this, "BBBCCCVVVS"),
      H(this, "longHashCode"),
      H(this, "versification"),
      H(this, "rtlMark", "‏"),
      H(this, "_bookNum", 0),
      H(this, "_chapterNum", 0),
      H(this, "_verseNum", 0),
      H(this, "_verse"),
      u == null && o == null)
    )
      if (e != null && typeof e == "string") {
        const a = e,
          p = r != null && r instanceof ut ? r : void 0;
        (this.setEmpty(p), this.parse(a));
      } else if (e != null && typeof e == "number") {
        const a = r != null && r instanceof ut ? r : void 0;
        (this.setEmpty(a),
          (this._verseNum = e % K.chapterDigitShifter),
          (this._chapterNum = Math.floor((e % K.bookDigitShifter) / K.chapterDigitShifter)),
        ...
        if (e != null && e instanceof K) {
          ...
          if (e == null) return;
          const a = e instanceof ut ? e : K.defaultVersification;
          this.setEmpty(a);
        }
      ...
      if (typeof e == "string" && typeof r == "string" && typeof u == "string")
        ...
        ((this._bookNum = e),
          (this._chapterNum = r),
          (this._verseNum = u),
          (this.versification = o ?? K.defaultVersification));
      else throw new Error("VerseRef constructor not supported.");
    else throw new Error("VerseRef constructor not supported.");
  }
  ...
  static tryParse(e) {
    ...
    try {
      ...
      if (u instanceof Tt) return ((r = new K()), { success: !1, verseRef: r });
      throw u;
    }
  }
  static getBBBCCCVVV(e, r, u) {
    return (
      (e % K.bcvMaxValue) * K.bookDigitShifter +
      (r >= 0 ? (r % K.bcvMaxValue) * K.chapterDigitShifter : 0) +
      (u >= 0 ? u % K.bcvMaxValue : 0)
    );
  }
  static fromJSON(e) {
    const { book: r, chapterNum: u, verseNum: o, verse: a, versificationStr: p } = e,
      h = a || o.toString();
    let f;
    return (p && (f = new ut(p)), r ? new K(r, u.toString(), h, f) : new K());
  }
  static tryGetVerseNum(e) {
    ...
    for (let o = 0; o < e.length; o++) {
      if (((u = e[o]), u < "0" || u > "9")) return (o === 0 && (r = -1), { success: !1, vNum: r });
      if (((r = r * 10 + +u - 0), r > K.bcvMaxValue)) return ((r = -1), { success: !1, vNum: r });
    }
  ...
  get hasMultiple() {
    return (
      this._verse != null &&
      (this._verse.includes(K.verseRangeSeparator) ||
        this._verse.includes(K.verseSequenceIndicator))
    );
  }
  get book() {
  ...
  set book(e) {
    this.bookNum = fe.bookIdToNumber(e);
  }
  get chapter() {
  ...
  set verse(e) {
    ...
    ((this._verse = r ? void 0 : e.replace(this.rtlMark, "")),
      (this._verseNum = u),
      !(this._verseNum >= 0) && ({ vNum: this._verseNum } = K.tryGetVerseNum(this._verse)));
  }
  get bookNum() {
    return this._bookNum;
  }
  set bookNum(e) {
    if (e <= 0 || e > fe.lastBook)
      throw new Tt("BookNum must be greater than zero and less than or equal to last book");
    this._bookNum = e;
  }
  get chapterNum() {
  ...
  parse(e) {
    if (((e = e.replace(this.rtlMark, "")), e.includes("/"))) {
      ...
      if (((e = a[0]), a.length > 1))
        try {
          const p = +a[1].trim();
          this.versification = new ut(Le[p]);
        } catch {
          throw new Tt("Invalid reference : " + e);
        }
    }
    const r = e.trim().split(" ");
    if (r.length !== 2) throw new Tt("Invalid reference : " + e);
    const u = r[1].split(":"),
      o = +u[0];
    if (
      u.length !== 2 ||
      fe.bookIdToNumber(r[0]) === 0 ||
      !Number.isInteger(o) ||
      o < 0 ||
      !K.isVerseParseable(u[1])
    )
      throw new Tt("Invalid reference : " + e);
      ...
      if ((o.push(h), p.length > 1)) {
        ...
        if (((d.verse = p[1]), !e))
          for (let m = f + 1; m < d.verseNum; m++) {
            const l = new K(this._bookNum, this._chapterNum, m, this.versification);
  ...
  get internalValid() {
    return this.versification == null
      ...
      : this._bookNum <= 0 || this._bookNum > fe.lastBook
        ? 2
        : (fe.isCanonical(this._bookNum), 0);
  }
  ...
  updateInternal(e, r, u) {
    ((this.bookNum = fe.bookIdToNumber(e)), (this.chapter = r), (this.verse = u));
  }
};
(H(ve, "defaultVersification", ut.English),
  H(ve, "verseRangeSeparator", "-"),
  H(ve, "verseSequenceIndicator", ","),
  H(ve, "verseRangeSeparators", [ve.verseRangeSeparator]),
  H(ve, "verseSequenceIndicators", [ve.verseSequenceIndicator]),
  H(ve, "chapterDigitShifter", 1e3),
  H(ve, "bookDigitShifter", ve.chapterDigitShifter * ve.chapterDigitShifter),
  H(ve, "bcvMaxValue", ve.chapterDigitShifter - 1),
  H(ve, "ValidStatusType", Hr));
let Jr = ve,
...
function At() {
  ...
  function t(R, F, U) {
    ...
    for (var ne = 0; ne < R.length; ne++)
      if (r(R, ne)) {
        var he = R[ne];
        if (F.call(void 0, he, ne, R)) return he;
      }
  }
  function e(R, F) {
    return (
      ...
      F &&
        ...
        (R = F.create(null, F.getOwnPropertyDescriptors(R))),
      F && typeof F.freeze == "function" ? F.freeze(R) : R
    );
  }
  function r(R, F) {
  ...
  var f = e({ script: !1, style: !1, textarea: !0, title: !0 });
  function d(R) {
    var F = R.toLowerCase();
    return r(f, F) && !f[F];
  }
  ...
  function v(R) {
    return l(R) || R === C.XML_XHTML_APPLICATION;
  }
  var C = e({
      HTML: "text/html",
    ...
    b = Object.keys(C).map(function (R) {
      return C[R];
    });
  function y(R) {
  ...
  return (
    ...
    (le.find = t),
    (le.freeze = e),
    (le.HTML_BOOLEAN_ATTRIBUTES = o),
    (le.HTML_RAW_TEXT_ELEMENTS = f),
    (le.HTML_VOID_ELEMENTS = p),
    (le.hasDefaultHTMLNamespace = v),
    (le.hasOwn = r),
    (le.isHTMLBooleanAttribute = a),
    (le.isHTMLRawTextElement = d),
    (le.isHTMLEscapableRawTextElement = m),
    (le.isHTMLMimeType = l),
    (le.isHTMLVoidElement = h),
    (le.isValidMimeType = y),
    (le.MIME_TYPE = C),
    (le.NAMESPACE = q),
    le
  );
...
function Pt() {
  ...
  function p(v, C) {
    (o(v)
      ? ((this.name = u[v]), (this.message = C || ""))
      : ((this.message = v), (this.name = a(C) ? C : r.Error)),
...
function Xr() {
  ...
  var e = t();
  function r(ue) {
    if (ue.source[0] !== "[") throw new Error(ue + " can not be used with chars");
  ...
  function u(ue, me) {
    ...
    if (ue.source.indexOf(me) === -1) throw new Error('"' + me + '" is not is /' + ue.source + "/");
  ...
  var p = "�",
    h = /[-\x09\x0A\x0D\x20-\x2C\x2E-\uD7FF\uE000-\uFFFD]/;
  e && (h = o("[", r(h), "\\u{10000}-\\u{10FFFF}", "]"));
  var f = /[\x20\x09\x0D\x0A]/,
    d = r(f),
    m = o(f, "+"),
    l = o(f, "*"),
    v =
  ...
  var C = r(v),
    ...
    te = a(/"[^"]*"|'[^']*'/),
    Re = o(/^<\?/, "(", y, ")", a(m, "(", h, "*?)"), "?", /\?>/),
    g = /[\x20\x0D\x0Aa-zA-Z0-9-'()+,./:=?;!*#@$_%]/,
    N = a('"', g, '*"', "|", "'", u(g, "'"), "*'"),
    S = "<!--",
    T = "-->",
    M = o(S, a(u(h, "-"), "|", o("-", u(h, "-"))), "*", T),
    A = "#PCDATA",
    B = a(o(/\(/, l, A, a(l, /\|/, l, W), "*", l, /\)\*/), "|", o(/\(/, l, A, l, /\)/)),
    X = /[?*+]?/,
    _ = o(/\([^>]+\)/, X),
    w = a("EMPTY", "|", "ANY", "|", B, "|", _),
    I = "<!ELEMENT",
    L = o(I, m, a(W, "|", ne), m, a(w, "|", ne), l, ">"),
    V = o("NOTATION", m, /\(/, l, y, a(l, /\|/, l, y), "*", l, /\)/),
    pe = o(/\(/, l, q, a(l, /\|/, l, q), "*", l, /\)/),
    we = a(V, "|", pe),
    Ae = a(/CDATA|ID|IDREF|IDREFS|ENTITY|ENTITIES|NMTOKEN|NMTOKENS/, "|", we),
    ae = a(/#REQUIRED|#IMPLIED/, "|", a(a("#FIXED", m), "?", E)),
    P = a(m, y, m, Ae, m, ae),
    Ue = "<!ATTLIST",
    Be = o(Ue, m, y, P, "*", l, ">"),
    de = "about:legacy-compat",
    Ve = a('"' + de + '"', "|", "'" + de + "'"),
    _e = "SYSTEM",
    Se = "PUBLIC",
    ke = a(a(_e, m, te), "|", a(Se, m, N, m, te)),
    Je = o(
      "^",
      a(
    ...
    bt = a(ze, "|", Ye),
    Ke = o(Se, m, N),
    We = o("<!NOTATION", m, y, m, a(ke, "|", Ke), l, ">"),
    $ = o(l, "=", l),
    Z = /1[.]\d+/,
    be = o(m, "version", $, a("'", Z, "'", "|", '"', Z, '"')),
    Ne = /[A-Za-z][-A-Za-z0-9._]*/,
    Qe = a(m, "encoding", $, a('"', Ne, '"', "|", "'", Ne, "'")),
    lt = a(
      m,
      "standalone",
      $,
  ...
  return (
    (j.chars = r),
    (j.chars_without = u),
    (j.detectUnicodeSupport = t),
    (j.reg = o),
    (j.regg = a),
    (j.ABOUT_LEGACY_COMPAT = de),
    (j.ABOUT_LEGACY_COMPAT_SystemLiteral = Ve),
    (j.AttlistDecl = Be),
    (j.CDATA_START = Nt),
    (j.CDATA_END = yt),
    (j.CDSect = Bt),
    ...
    (j.COMMENT_START = S),
    (j.COMMENT_END = T),
    (j.DOCTYPE_DECL_START = ht),
    (j.elementdecl = L),
    (j.EntityDecl = bt),
    (j.EntityValue = he),
    (j.ExternalID = ke),
    (j.ExternalID_match = Je),
    (j.Name = y),
    (j.NotationDecl = We),
    (j.Reference = U),
    (j.PEReference = ne),
    ...
    (j.SYSTEM = _e),
    (j.SystemLiteral = te),
    (j.UNICODE_REPLACEMENT_CHARACTER = p),
    (j.UNICODE_SUPPORT = e),
    (j.XMLDecl = ft),
    j
  );
...
function Yr() {
  ...
  var t = At(),
    e = t.find,
    r = t.hasDefaultHTMLNamespace,
    u = t.hasOwn,
    o = t.isHTMLMimeType,
    a = t.isHTMLRawTextElement,
    p = t.isHTMLVoidElement,
    h = t.MIME_TYPE,
    f = t.NAMESPACE,
    d = Symbol(),
    m = Pt(),
    l = m.DOMException,
    v = m.DOMExceptionName,
    C = Xr();
  function b(n) {
    if (n !== d) throw new TypeError("Illegal constructor");
  }
  function y(n) {
    return n !== "";
  }
  ...
  function x(n, i) {
    ...
    s.constructor != n &&
      (typeof n != "function" && console.error("unknown Class:" + n), (s.constructor = n));
  }
  var k = {},
    G = (k.ELEMENT_NODE = 1),
    W = (k.ATTRIBUTE_NODE = 2),
    ee = (k.TEXT_NODE = 3),
    xe = (k.CDATA_SECTION_NODE = 4),
    te = (k.ENTITY_REFERENCE_NODE = 5),
    Re = (k.ENTITY_NODE = 6),
    g = (k.PROCESSING_INSTRUCTION_NODE = 7),
    N = (k.COMMENT_NODE = 8),
    S = (k.DOCUMENT_NODE = 9),
    T = (k.DOCUMENT_TYPE_NODE = 10),
    M = (k.DOCUMENT_FRAGMENT_NODE = 11),
    A = (k.NOTATION_NODE = 12),
    B = t.freeze({
  ...
  ((w.prototype = {
    ...
    toString: function (n) {
      for (var i = [], s = 0; s < this.length; s++) qe(this[s], i, n);
      return i.join("");
    },
    filter: function (n) {
    ...
    (w.prototype[Symbol.iterator] = function () {
      ...
      return {
        next: function () {
          return i < n.length ? { value: n[i++], done: !1 } : { done: !0 };
        },
        return: function () {
          return { done: !0 };
        },
  ...
  function L(n) {
    ...
    if (n._inc !== i) {
      ...
      if ((ur(n, "length", s.length), !n.$$length || s.length < n.$$length))
        for (var c = s.length; c in n; c++) u(n, c) && delete n[c];
      (E(s, n), (n._inc = i));
    }
  }
  ...
  function Ae(n, i, s) {
    var c = pe(i, s);
    if (c >= 0) {
      for (var D = i.length - 1; c <= D; ) i[c] = i[++c];
      if (((i.length = D), n)) {
  ...
  ((V.prototype = {
    ...
    setNamedItem: function (n) {
      ...
      if (i && i !== this._ownerElement) throw new l(l.INUSE_ATTRIBUTE_ERR);
    ...
    getNamedItemNS: function (n, i) {
      ...
      for (var s = 0; s < this.length; ) {
        var c = this[s];
        if (c.localName === i && c.namespaceURI === n) return c;
        s++;
      }
      return null;
    },
  ...
  ae.prototype = {
    ...
    createHTMLDocument: function (n) {
      ...
      if (((i.implementation = this), (i.childNodes = new w()), n !== !1)) {
        ((i.doctype = this.createDocumentType("html")),
          (i.doctype.ownerDocument = i),
          i.appendChild(i.doctype));
  ...
  P.prototype = {
    ...
    isEqualNode: function (n) {
      ...
      switch (this.nodeType) {
        case this.DOCUMENT_TYPE_NODE:
          if (this.name !== n.name || this.publicId !== n.publicId || this.systemId !== n.systemId)
            return !1;
          break;
        case this.ELEMENT_NODE:
          ...
          for (var i = 0; i < this.attributes.length; i++) {
            ...
            if (!s.isEqualNode(n.getAttributeNodeNS(s.namespaceURI, s.localName))) return !1;
          }
          break;
        ...
        case this.PROCESSING_INSTRUCTION_NODE:
          if (this.target !== n.target || this.data !== n.data) return !1;
          break;
        case this.TEXT_NODE:
      ...
      if (this.childNodes.length !== n.childNodes.length) return !1;
      for (var i = 0; i < this.childNodes.length; i++)
        if (!this.childNodes[i].isEqualNode(n.childNodes[i])) return !1;
      return !0;
    },
    ...
    replaceChild: function (n, i) {
      ($(this, n, i, We), i && this.removeChild(i));
    },
    ...
    lookupPrefix: function (n) {
      for (var i = this; i; ) {
        var s = i._nsMap;
        if (s) {
          for (var c in s) if (u(s, c) && s[c] === n) return c;
        }
    ...
    compareDocumentPosition: function (n) {
      ...
      var i = n,
        ...
        D = null;
      if (
        (i instanceof be && ((c = i), (i = c.ownerElement)),
        s instanceof be && ((D = s), (s = D.ownerElement), c && i && s === i))
      )
      ...
      if (!i || !s || s.ownerDocument !== i.ownerDocument)
        return (
          B.DOCUMENT_POSITION_DISCONNECTED +
          B.DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC +
          (_(s.ownerDocument) > _(i.ownerDocument)
            ? B.DOCUMENT_POSITION_FOLLOWING
            : B.DOCUMENT_POSITION_PRECEDING)
        );
      if (D && i === s) return B.DOCUMENT_POSITION_CONTAINS + B.DOCUMENT_POSITION_PRECEDING;
      if (c && i === s) return B.DOCUMENT_POSITION_CONTAINED_BY + B.DOCUMENT_POSITION_FOLLOWING;
      for (var ce = [], ge = i.parentNode; ge; ) {
        if (!D && ge === s) return B.DOCUMENT_POSITION_CONTAINED_BY + B.DOCUMENT_POSITION_FOLLOWING;
        (ce.push(ge), (ge = ge.parentNode));
      }
      ce.reverse();
      for (var Ce = [], De = s.parentNode; De; ) {
        if (!c && De === i) return B.DOCUMENT_POSITION_CONTAINS + B.DOCUMENT_POSITION_PRECEDING;
        (Ce.push(De), (De = De.parentNode));
      }
      Ce.reverse();
      var et = X(ce, Ce);
      for (var $e in et.childNodes) {
        var Oe = et.childNodes[$e];
        ...
        if (ce.indexOf(Oe) >= 0) return B.DOCUMENT_POSITION_PRECEDING;
      }
      return 0;
    },
  };
  ...
  function de(n, i) {
    ...
    ((this.ownerDocument = this),
      (this.contentType = s.contentType || h.XML_APPLICATION),
      (this.type = o(this.contentType) ? "html" : "xml"));
  }
  function Ve(n, i, s) {
    n && n._inc++;
  ...
  function Se(n, i, s) {
    if (n && n._inc) {
      n._inc++;
      var c = i.childNodes;
      if (s && !s.nextSibling) c[c.length++] = s;
      else {
        for (var D = i.firstChild, O = 0; D; ) ((c[O++] = D), (D = D.nextSibling));
        ((c.length = O), delete c[c.length]);
      }
    }
  }
  function ke(n, i) {
    if (n !== i.parentNode) throw new l(l.NOT_FOUND_ERR, "child's parent is not parent");
    var s = i.previousSibling,
      c = i.nextSibling;
    return (
      s ? (s.nextSibling = c) : (n.firstChild = c),
      c ? (c.previousSibling = s) : (n.lastChild = s),
      Se(n.ownerDocument, n),
      (i.parentNode = null),
  ...
  function Je(n) {
    return (
      n &&
      (n.nodeType === P.DOCUMENT_NODE ||
        n.nodeType === P.DOCUMENT_FRAGMENT_NODE ||
        n.nodeType === P.ELEMENT_NODE)
    );
  }
  function Xe(n) {
    return (
      n &&
      (n.nodeType === P.CDATA_SECTION_NODE ||
        n.nodeType === P.COMMENT_NODE ||
        n.nodeType === P.DOCUMENT_FRAGMENT_NODE ||
        n.nodeType === P.DOCUMENT_TYPE_NODE ||
        n.nodeType === P.ELEMENT_NODE ||
        n.nodeType === P.PROCESSING_INSTRUCTION_NODE ||
        n.nodeType === P.TEXT_NODE)
    );
  }
  function ye(n) {
  ...
  function bt(n, i, s) {
    if (!Je(n)) throw new l(l.HIERARCHY_REQUEST_ERR, "Unexpected parent node type " + n.nodeType);
    if (s && s.parentNode !== n) throw new l(l.NOT_FOUND_ERR, "child not in parent");
    if (!Xe(i) || (ye(i) && n.nodeType !== P.DOCUMENT_NODE))
      throw new l(
  ...
  function Ke(n, i, s) {
    ...
    if (ye(i)) {
      if (e(c, ye)) throw new l(l.HIERARCHY_REQUEST_ERR, "Only one doctype is allowed");
      var Y = e(c, Q);
  ...
  function $(n, i, s, c) {
    (bt(n, i, s), n.nodeType === P.DOCUMENT_NODE && (c || Ke)(n, i, s));
    var D = i.parentNode;
    ...
    ((O.previousSibling = ce),
      (Y.nextSibling = s),
      ce ? (ce.nextSibling = O) : (n.firstChild = O),
      s == null ? (n.lastChild = Y) : (s.previousSibling = Y));
    do O.parentNode = n;
    while (O !== Y && (O = O.nextSibling));
    return (
      Se(n.ownerDocument || n, n, i),
      i.nodeType == M && (i.firstChild = i.lastChild = null),
      i
    );
  }
  ((de.prototype = {
    ...
    insertBefore: function (n, i) {
      ...
      return (
        ...
        this.documentElement === null && n.nodeType === G && (this.documentElement = n),
        n
      );
    },
    removeChild: function (n) {
    ...
    getElementById: function (n) {
      var i = null;
      return (
        Be(this.documentElement, function (s) {
          if (s.nodeType == G && s.getAttribute("id") == n) return ((i = s), !0);
        }),
        i
      );
    },
    createElement: function (n) {
      ...
      ((i.ownerDocument = this),
        this.type === "html" && (n = n.toLowerCase()),
        r(this.contentType) && (i.namespaceURI = f.HTML),
        (i.nodeName = n),
        (i.tagName = n),
        (i.localName = n),
        (i.childNodes = new w()));
    ...
    createProcessingInstruction: function (n, i) {
      ...
      return (
        ...
        (s.childNodes = new w()),
        (s.nodeName = s.target = n),
        (s.nodeValue = s.data = i),
        s
      );
    },
    ...
    createEntityReference: function (n) {
      if (!C.Name.test(n)) throw new l(l.INVALID_CHARACTER_ERR, 'not a valid xml name "' + n + '"');
      if (this.type === "html") throw new l("document is an html document", v.NotSupportedError);
      var i = new pt(d);
    ...
    x(de, P));
  function Z(n) {
    (b(n), (this._nsMap = Object.create(null)));
  }
  ((Z.prototype = {
    nodeType: G,
    attributes: null,
    ...
    getAttributeNode: function (n) {
      return (
        ...
        this.attributes.getNamedItem(n)
      );
    },
    setAttribute: function (n, i) {
      ...
      s
        ...
        : ((s = this.ownerDocument._createAttribute(n)),
          (s.value = s.nodeValue = "" + i),
    ...
    setAttributeNS: function (n, i, s) {
      var c = he(n, i),
        D = c[2],
        O = this.getAttributeNodeNS(n, D);
      O
        ? (O.value = O.nodeValue = "" + s)
        : ((O = this.ownerDocument.createAttributeNS(n, i)),
          (O.value = O.nodeValue = "" + s),
    ...
    getElementsByTagName: function (n) {
      var i = (this.nodeType === S ? this : this.ownerDocument).type === "html",
        s = n.toLowerCase();
          ...
          Be(c, function (O) {
            if (!(O === c || O.nodeType !== G))
              if (n === "*") D.push(O);
                ...
                var Y = O.getQualifiedName(),
                  ce = i && O.namespaceURI === f.HTML ? s : n;
                Y === ce && D.push(O);
              }
          }),
          D
    ...
    (de.prototype.getElementsByClassName = Z.prototype.getElementsByClassName),
    (de.prototype.getElementsByTagName = Z.prototype.getElementsByTagName),
    (de.prototype.getElementsByTagNameNS = Z.prototype.getElementsByTagNameNS),
    x(Z, P));
  function be(n) {
  ...
  ((Ne.prototype = {
    ...
    replaceData: function (n, i, s) {
      var c = this.data.substring(0, n),
        D = this.data.substring(n + i);
  ...
  ((lt.prototype = { nodeName: "#comment", nodeType: N }), x(lt, Ne));
  function ft(n) {
    b(n);
  }
  ((ft.prototype = { nodeName: "#cdata-section", nodeType: xe }), x(ft, Qe));
  function ht(n) {
    b(n);
  }
  ((ht.prototype.nodeType = T), x(ht, P));
  function Nt(n) {
    b(n);
  }
  ((Nt.prototype.nodeType = A), x(Nt, P));
  function yt(n) {
    b(n);
  }
  ((yt.prototype.nodeType = Re), x(yt, P));
  function pt(n) {
    b(n);
  }
  ((pt.prototype.nodeType = te), x(pt, P));
  function Ze(n) {
    b(n);
  }
  ((Ze.prototype.nodeName = "#document-fragment"), (Ze.prototype.nodeType = M), x(Ze, P));
  function dt(n) {
    b(n);
  }
  ((dt.prototype.nodeType = g), x(dt, Ne));
  function Bt() {}
  ((Bt.prototype.serializeToString = function (n, i) {
    return ue.call(n, i);
  }),
    (P.prototype.toString = ue));
  function ue(n) {
    var i = [],
      s = (this.nodeType === S && this.documentElement) || this,
      c = s.prefix,
      D = s.namespaceURI;
    if (D && c == null) {
      var c = s.lookupPrefix(D);
      if (c == null) var O = [{ namespace: D, prefix: null }];
    }
  ...
  function me(n, i, s) {
    var c = n.prefix || "",
      D = n.namespaceURI;
    if (!D || (c === "xml" && D === f.XML) || D === f.XMLNS) return !1;
    for (var O = s.length; O--; ) {
      var Y = s[O];
      if (Y.prefix === c) return Y.namespace !== D;
    }
    return !0;
  }
  function Fe(n, i, s) {
  ...
  function qe(n, i, s, c) {
    c || (c = []);
    var D = n.nodeType === S ? n : n.ownerDocument,
      O = D.type === "html";
    if (s)
      if (((n = s(n)), n)) {
        if (typeof n == "string") {
    ...
    switch (n.nodeType) {
      case G:
        ...
        for (var tt = 0; tt < ce; tt++) {
          var Pe = Y.item(tt);
          Pe.prefix == "xmlns"
            ? c.push({ prefix: Pe.localName, namespace: Pe.value })
        ...
        for (var tt = 0; tt < ce; tt++) {
          ...
          if (me(Pe, O, c)) {
            var rt = Pe.prefix || "",
              Ct = Pe.namespaceURI;
            (Fe(i, rt ? "xmlns:" + rt : "xmlns", Ct), c.push({ prefix: rt, namespace: Ct }));
          }
          qe(Pe, i, s, c);
        }
        if (ge === Ce && me(n, O, c)) {
        ...
        var qt = !Ee;
        if ((qt && (O || n.namespaceURI === f.HTML) && (qt = p(ge)), qt)) i.push("/>");
        else {
          if ((i.push(">"), O && a(ge)))
            for (; Ee; )
              (Ee.data ? i.push(Ee.data) : qe(Ee, i, s, c.slice()), (Ee = Ee.nextSibling));
          else for (; Ee; ) (qe(Ee, i, s, c.slice()), (Ee = Ee.nextSibling));
          i.push("</", Ce, ">");
        }
        return;
      case S:
      case M:
        for (var Ee = n.firstChild; Ee; ) (qe(Ee, i, s, c.slice()), (Ee = Ee.nextSibling));
        return;
      case W:
      ...
      case T:
        var ir = n.publicId,
          mt = n.systemId;
        (i.push(C.DOCTYPE_DECL_START, " ", n.name),
          ir
            ? (i.push(" ", C.PUBLIC, " ", ir), mt && mt !== "." && i.push(" ", mt))
            : mt && mt !== "." && i.push(" ", C.SYSTEM, " ", mt),
          n.internalSubset && i.push(" [", n.internalSubset, "]"),
          i.push(">"));
        return;
      case g:
  ...
  function nr(n, i, s) {
    ...
    if ((c || (c = i.cloneNode(!1)), (c.ownerDocument = n), (c.parentNode = null), s))
      for (var D = i.firstChild; D; ) (c.appendChild(nr(n, D, s)), (D = D.nextSibling));
    return c;
  }
  function Ft(n, i, s) {
    ...
    switch ((i.childNodes && (c.childNodes = new w()), (c.ownerDocument = n), c.nodeType)) {
      case G:
        var Y = i.attributes,
          ce = (c.attributes = new V()),
          ge = Y.length;
        ce._ownerElement = c;
        for (var Ce = 0; Ce < ge; Ce++) c.setAttributeNode(Ft(n, Y.item(Ce), !0));
        break;
      case W:
        s = !0;
    }
    if (s) for (var De = i.firstChild; De; ) (c.appendChild(Ft(n, De, s)), (De = De.nextSibling));
    return c;
  }
  function ur(n, i, s) {
    n[i] = s;
  ...
  return (
    (re._updateLiveList = L),
    (re.Attr = be),
    (re.CDATASection = ft),
    (re.CharacterData = Ne),
    (re.Comment = lt),
    (re.Document = de),
    (re.DocumentFragment = Ze),
    (re.DocumentType = ht),
    (re.DOMImplementation = ae),
    (re.Element = Z),
    (re.Entity = yt),
    (re.EntityReference = pt),
    (re.LiveNodeList = I),
    (re.NamedNodeMap = V),
    (re.Node = P),
    (re.NodeList = w),
...
function cu() {
  ...
  var t = At(),
    e = Xr(),
    r = Pt(),
    u = t.isHTMLEscapableRawTextElement,
    o = t.isHTMLMimeType,
    a = t.isHTMLRawTextElement,
    p = t.hasOwn,
    h = t.NAMESPACE,
  ...
  F.prototype = {
    parse: function (g, N, S) {
      ...
      (T.startDocument(),
        G(N, (N = Object.create(null))),
        ne(g, N, S, T, this.errorHandler),
        T.endDocument());
    },
  };
  var U = /&#?\w+;?/g;
  function ne(g, N, S, T, M) {
    var A = o(T.mimeType);
    g.indexOf(e.UNICODE_REPLACEMENT_CHARACTER) >= 0 &&
      M.warning("Unicode replacement character detected, source encoding issues?");
    function B($) {
      if ($ > 65535) {
        $ -= 65536;
        var Z = 55296 + ($ >> 10),
    ...
    function X($) {
      var Z = $[$.length - 1] === ";" ? $ : $ + ";";
      if (!A && Z !== $) return (M.error("EntityRef: expecting ;"), $);
      ...
      return p(S, Ne)
        ? S[Ne]
        : Ne.charAt(0) === "#"
          ? B(parseInt(Ne.substring(1).replace("x", "0x")))
          : (M.error("entity not found:" + $), $);
    }
    function _($) {
      if ($ > ae) {
    ...
    var w = 0,
      ...
      V = T.locator;
    function pe($, Z) {
      for (; $ >= I && (Z = L.exec(g)); ) ((w = I), (I = Z.index + Z[0].length), V.lineNumber++);
      V.columnNumber = $ - w + 1;
    }
    for (var we = [{ currentNSMap: N }], Ae = [], ae = 0; ; ) {
      try {
        var P = g.indexOf("<", ae);
        if (P < 0) {
          if (!A && Ae.length > 0) return M.fatalError("unclosed xml tag(s): " + Ae.join(", "));
          if (!g.substring(ae).match(/^\s*$/)) {
            var Ue = T.doc,
              Be = Ue.createTextNode(g.substring(ae));
            if (Ue.documentElement) return M.error("Extra content at the end of the document");
            (Ue.appendChild(Be), (T.currentElement = Be));
          }
          return;
        }
        if (P > ae) {
          var de = g.substring(ae, P);
          (!A &&
            Ae.length === 0 &&
            ((de = de.replace(new RegExp(e.S_OPT.source, "g"), "")),
            de && M.error("Unexpected content outside root element: '" + de + "'")),
            _(P));
        }
        switch (g.charAt(P + 1)) {
          case "/":
            var se = g.indexOf(">", P + 2),
              Ve = g.substring(P + 2, se > 0 ? se : void 0);
            if (!Ve) return M.fatalError("end tag name missing");
            var _e = se > 0 && e.reg("^", e.QName_group, e.S_OPT, "$").exec(Ve);
            if (!_e) return M.fatalError('end tag name contains invalid characters: "' + Ve + '"');
            if (!T.currentElement && !T.doc.documentElement) return;
            var Se =
              Ae[Ae.length - 1] || T.currentElement.tagName || T.doc.documentElement.tagName || "";
            if (Se !== _e[1]) {
              var ke = _e[1].toLowerCase();
              if (!A || Se.toLowerCase() !== ke)
                return M.fatalError(
                  'Opening and ending tag mismatch: "' + Se + '" != "' + Ve + '"',
                );
            }
            var Je = we.pop();
            Ae.pop();
            ...
            if ((T.endElement(Je.uri, Je.localName, Se), Xe))
              for (var ye in Xe) p(Xe, ye) && T.endPrefixMapping(ye);
            se++;
            break;
          case "?":
            (V && pe(P), (se = te(g, P, T, M)));
            break;
          case "!":
            (V && pe(P), (se = xe(g, P, T, M, A)));
            break;
          default:
            V && pe(P);
            var Q = new Re(),
              ze = we[we.length - 1].currentNSMap,
              se = E(g, P, Q, ze, X, M, A),
              Ye = Q.length;
            if (
              (Q.closed ||
                (A && t.isHTMLVoidElement(Q.tagName) ? (Q.closed = !0) : Ae.push(Q.tagName)),
              V && Ye)
            ) {
              for (var bt = he(V, {}), Ke = 0; Ke < Ye; Ke++) {
                var We = Q[Ke];
                (pe(We.offset), (We.locator = he(V, {})));
              }
              ((T.locator = bt), x(Q, T, ze) && we.push(Q), (T.locator = V));
            } else x(Q, T, ze) && we.push(Q);
            A && !Q.closed ? (se = k(g, se, Q.tagName, X, T)) : se++;
        }
      } catch ($) {
        if ($ instanceof f) throw $;
        if ($ instanceof d) throw new f($.name + ": " + $.message, T.locator, $);
  ...
  function E(g, N, S, T, M, A, B) {
    function X(pe, we, Ae) {
      if (p(S.attributeNames, pe)) return A.fatalError("Attribute " + pe + " redefined");
      if (!B && we.indexOf("<") >= 0)
        return A.fatalError("Unescaped '<' not allowed in attributes values");
      S.addValue(pe, we.replace(/[\t\n\r]/g, " ").replace(U, M), Ae);
    }
    for (var _, w, I = ++N, L = m; ; ) {
      var V = g.charAt(I);
      switch (V) {
        case "=":
          if (L === l) ((_ = g.slice(N, I)), (L = C));
          else if (L === v) L = C;
        ...
        case '"':
          if (L === C || L === l)
            if (
              (L === l && (A.warning('attribute value must after "="'), (_ = g.slice(N, I))),
              (N = I + 1),
              (I = g.indexOf(V, N)),
              I > 0)
            )
              ((w = g.slice(N, I)), X(_, w, N - 1), (L = y));
            else throw new Error("attribute value no end '" + V + "' match");
          else if (L == b)
            ((w = g.slice(N, I)),
              X(_, w, N),
              A.warning('attribute "' + _ + '" missed start quot(' + V + ")!!"),
              (N = I + 1),
              (L = y));
            ...
            case v:
              (L === v && (w = _),
                L == b
                  ...
                  : (B || A.warning('attribute "' + w + '" missed value!! "' + w + '" instead!!'),
                    X(w, w, N)));
              break;
            case C:
              if (!B) return A.fatalError(`AttValue: ' or " expected`);
          }
          return I;
        case "":
          V = " ";
        default:
          if (V <= " ")
            ...
            switch (L) {
              case v:
                (B || A.warning('attribute "' + _ + '" missed value!! "' + _ + '" instead2!!'),
                  X(_, _, N),
                  (N = I),
                  (L = l));
                break;
  ...
  function x(g, N, S) {
    for (var T = g.tagName, M = null, L = g.length; L--; ) {
      var A = g[L],
        ...
        V = B.indexOf(":");
      if (V > 0)
        var _ = (A.prefix = B.slice(0, V)),
          w = B.slice(V + 1),
          I = _ === "xmlns" && w;
      else ((w = B), (_ = null), (I = B === "xmlns" && ""));
      ((A.localName = w),
        I !== !1 &&
          (M == null && ((M = Object.create(null)), G(S, (S = Object.create(null)))),
          (S[I] = M[I] = X),
          (A.uri = h.XMLNS),
          N.startPrefixMapping(I, X)));
    }
    for (var L = g.length; L--; )
      ((A = g[L]),
        A.prefix &&
          (A.prefix === "xml" && (A.uri = h.XML), A.prefix !== "xmlns" && (A.uri = S[A.prefix])));
    var V = T.indexOf(":");
    V > 0
      ? ((_ = g.prefix = T.slice(0, V)), (w = g.localName = T.slice(V + 1)))
      : ((_ = null), (w = g.localName = T));
    var pe = (g.uri = S[_ || ""]);
    if ((N.startElement(pe, w, T, g), g.closed)) {
      if ((N.endElement(pe, w, T), M)) for (_ in M) p(M, _) && N.endPrefixMapping(_);
    } else return ((g.currentNSMap = S), (g.localNSMap = M), !0);
  }
  function k(g, N, S, T, M) {
    ...
    if (A || a(S)) {
      var B = g.indexOf("</" + S + ">", N),
        X = g.substring(N + 1, B);
      return (A && (X = X.replace(U, T)), M.characters(X, 0, X.length), B);
    }
    return N + 1;
  }
  function G(g, N) {
    for (var S in g) p(g, S) && (N[S] = g[S]);
  }
  function W(g, N) {
    var S = N;
    function T(I) {
...
` &&
    ...
    function S(X, _) {
      ...
      return w
        ? w[1].toLowerCase() === "xml"
          ...
          : (X.skip(w[0].length), w[0])
        : _.fatalError("processing instruction is not well-formed at position " + X.getIndex());
    }
    var T = g.getSource();
    if (g.char() === "[") {
      g.skip(1);
      for (var M = g.getIndex(); g.getIndex() < T.length; ) {
        ...
        if (g.char() === "<" && g.char(1) === "!")
          switch (g.char(2)) {
            case "E":
              g.char(3) === "L"
                ? (B = g.getMatch(e.elementdecl))
                : g.char(3) === "N" && (B = g.getMatch(e.EntityDecl));
              break;
            case "A":
        ...
        else if (g.char() === "<" && g.char(1) === "?") B = S(g, N);
        else if (g.char() === "%") B = g.getMatch(e.PEReference);
        else return N.fatalError("Error detected in Markup declaration");
        if (!B) return N.fatalError("Error in internal subset at position " + g.getIndex());
      }
      return N.fatalError("doctype internal subset is not well-formed, missing ]");
    }
  }
  function xe(g, N, S, T, M) {
    var A = W(g, N);
    switch (M ? A.char(2).toUpperCase() : A.char(2)) {
      case "-":
        ...
        return B
          ? (S.comment(
              B,
              e.COMMENT_START.length,
              B.length - e.COMMENT_START.length - e.COMMENT_END.length,
            ),
            A.getIndex())
          : T.fatalError("comment is not well-formed at position " + A.getIndex());
      case "[":
        var X = A.getMatch(e.CDSect);
        return X
          ? !M && !S.currentElement
            ? T.fatalError("CDATA outside of element")
            : (S.startCDATA(),
              S.characters(
                X,
                e.CDATA_START.length,
                X.length - e.CDATA_START.length - e.CDATA_END.length,
              ),
              S.endCDATA(),
              A.getIndex())
          : T.fatalError("Invalid CDATA starting at position " + N);
      case "D": {
        if (S.doc && S.doc.documentElement)
          return T.fatalError(
            "Doctype not allowed inside or after documentElement at position " + A.getIndex(),
          );
        if (
          M
            ? !A.substringStartsWithCaseInsensitive(e.DOCTYPE_DECL_START)
            : !A.substringStartsWith(e.DOCTYPE_DECL_START)
        )
        ...
        if ((A.skip(e.DOCTYPE_DECL_START.length), A.skipBlanks() < 1))
          return T.fatalError(
            "Expected whitespace after " + e.DOCTYPE_DECL_START + " at position " + A.getIndex(),
          );
        var _ = { name: void 0, publicId: void 0, systemId: void 0, internalSubset: void 0 };
        if (((_.name = A.getMatch(e.Name)), !_.name))
          return T.fatalError(
            "doctype name missing or contains unexpected characters at position " + A.getIndex(),
          );
        if (
          (M &&
            _.name.toLowerCase() !== "html" &&
            T.warning("Unexpected DOCTYPE in HTML document at position " + A.getIndex()),
          A.skipBlanks(),
          A.substringStartsWith(e.PUBLIC) || A.substringStartsWith(e.SYSTEM))
        ) {
          var w = e.ExternalID_match.exec(A.substringFromIndex());
          if (!w)
            return T.fatalError(
              "doctype external id is not well-formed at position " + A.getIndex(),
            );
          (w.groups.SystemLiteralOnly !== void 0
            ? (_.systemId = w.groups.SystemLiteralOnly)
            : ((_.systemId = w.groups.SystemLiteral), (_.publicId = w.groups.PubidLiteral)),
            A.skip(w[0].length));
        } else if (M && A.substringStartsWithCaseInsensitive(e.SYSTEM)) {
          if ((A.skip(e.SYSTEM.length), A.skipBlanks() < 1))
            return T.fatalError(
              "Expected whitespace after " + e.SYSTEM + " at position " + A.getIndex(),
            );
          if (((_.systemId = A.getMatch(e.ABOUT_LEGACY_COMPAT_SystemLiteral)), !_.systemId))
            return T.fatalError(
              "Expected " +
                e.ABOUT_LEGACY_COMPAT +
        ...
        return (
          M &&
            _.systemId &&
            !e.ABOUT_LEGACY_COMPAT_SystemLiteral.test(_.systemId) &&
            T.warning("Unexpected doctype.systemId in HTML document at position " + A.getIndex()),
          M || (A.skipBlanks(), (_.internalSubset = ee(A, T))),
          A.skipBlanks(),
          A.char() !== ">"
            ? T.fatalError("doctype not terminated with > at position " + A.getIndex())
            : (A.skip(1),
              S.startDTD(_.name, _.publicId, _.systemId, _.internalSubset),
              S.endDTD(),
              A.getIndex())
        );
      }
  ...
  function te(g, N, S, T) {
    ...
    if (!M) return T.fatalError("Invalid processing instruction starting at position " + N);
  ...
  return (
    (Re.prototype = {
      setTagName: function (g) {
        if (!e.QName_exact.test(g)) throw new Error("invalid tagName:" + g);
        this.tagName = g;
      },
      addValue: function (g, N, S) {
        if (!e.QName_exact.test(g)) throw new Error("invalid attribute:" + g);
        ((this.attributeNames[g] = this.length),
          (this[this.length++] = { qName: g, value: N, offset: S }));
      },
      length: 0,
    ...
    (wt.XMLReader = F),
    (wt.parseUtils = W),
    (wt.parseDoctypeCommentOrCData = xe),
    wt
  );
}
var Er;
function lu() {
  ...
  var t = At(),
    ...
    u = su(),
    o = cu(),
    a = e.DOMImplementation,
    p = t.hasDefaultHTMLNamespace,
    h = t.isHTMLMimeType,
    f = t.isValidMimeType,
    d = t.MIME_TYPE,
    m = t.NAMESPACE,
...
`,
  ...
  function b(E) {
    if (
      ((E = E || {}),
      E.locator === void 0 && (E.locator = !0),
      (this.assign = E.assign || t.assign),
      (this.domHandler = E.domHandler || y),
    ...
    (E.errorHandler &&
      ...
      (this.normalizeLineEndings = E.normalizeLineEndings || C),
  ...
  b.prototype.parseFromString = function (E, x) {
    if (!f(x))
      throw new TypeError(
        'DOMParser.parseFromString: the provided mimeType "' + x + '" is not valid.',
      );
    var k = this.assign(Object.create(null), this.xmlns),
      G = u.XML_ENTITIES,
      W = k[""] || null;
    (p(x) ? ((G = u.HTML_ENTITIES), (W = m.HTML)) : x === d.XML_SVG_IMAGE && (W = m.SVG),
      (k[""] = W),
      (k.xml = k.xml || m.XML));
    var ee = new this.domHandler({ mimeType: x, defaultNamespace: W, onError: this.onError }),
    ...
    return (
      Re && typeof E != "string" && te.errorHandler.fatalError("source is not a string"),
      te.parse(this.normalizeLineEndings(String(E)), k, G),
      ee.doc.documentElement || te.errorHandler.fatalError("missing root element"),
      ee.doc
    );
  };
  function y(E) {
    var x = E || {};
    ((this.mimeType = x.mimeType || d.XML_APPLICATION),
      (this.defaultNamespace = x.defaultNamespace || null),
      (this.cdata = !1),
      (this.currentElement = void 0),
      (this.doc = void 0),
      (this.locator = void 0),
      (this.onError = x.onError));
  }
  function q(E, x) {
  ...
  y.prototype = {
    startDocument: function () {
      var E = new a();
      this.doc = h(this.mimeType)
        ? E.createHTMLDocument(!1)
        : E.createDocument(this.defaultNamespace, "");
    },
    startElement: function (E, x, k, G) {
      var W = this.doc,
        ee = W.createElementNS(E, k || x),
        xe = G.length;
      ...
      for (var te = 0; te < xe; te++) {
        var E = G.getURI(te),
          Re = G.getValue(te),
          k = G.getQName(te),
          g = W.createAttributeNS(E, k);
        (this.locator && q(G.getLocator(te), g),
          (g.value = g.nodeValue = Re),
          ee.setAttributeNode(g));
      }
    },
    ...
    characters: function (E, x, k) {
      if (((E = F.apply(this, arguments)), E)) {
        if (this.cdata) var G = this.doc.createCDATASection(E);
        else var G = this.doc.createTextNode(E);
        (this.currentElement
          ? this.currentElement.appendChild(G)
          : /^\s*$/.test(E) && this.doc.appendChild(G),
          this.locator && q(this.locator, G));
      }
    },
    ...
    reportError: function (E, x) {
      ...
      else console.error("[xmldom " + E + "]	" + x, R(this.locator));
    },
    warning: function (E) {
...
@#[line:` +
  ...
  return (
    (nt.__DOMHandler = y),
    (nt.DOMParser = b),
    (nt.normalizeLineEndings = C),
    (nt.onErrorStopParsing = ne),
    (nt.onWarningStopParsing = he),
    nt
  );
}
var Dr;
function fu() {
  ...
  var t = At();
  ((J.assign = t.assign),
    (J.hasDefaultHTMLNamespace = t.hasDefaultHTMLNamespace),
    (J.isHTMLMimeType = t.isHTMLMimeType),
    (J.isValidMimeType = t.isValidMimeType),
    (J.MIME_TYPE = t.MIME_TYPE),
    (J.NAMESPACE = t.NAMESPACE));
  var e = Pt();
  ((J.DOMException = e.DOMException),
    (J.DOMExceptionName = e.DOMExceptionName),
    (J.ExceptionCode = e.ExceptionCode),
    (J.ParseError = e.ParseError));
  var r = Yr();
  ((J.Attr = r.Attr),
    (J.CDATASection = r.CDATASection),
    (J.CharacterData = r.CharacterData),
    (J.Comment = r.Comment),
    (J.Document = r.Document),
    (J.DocumentFragment = r.DocumentFragment),
    (J.DocumentType = r.DocumentType),
    (J.DOMImplementation = r.DOMImplementation),
    (J.Element = r.Element),
    (J.Entity = r.Entity),
    (J.EntityReference = r.EntityReference),
    (J.LiveNodeList = r.LiveNodeList),
    (J.NamedNodeMap = r.NamedNodeMap),
    (J.Node = r.Node),
    (J.NodeList = r.NodeList),
    (J.Notation = r.Notation),
    (J.ProcessingInstruction = r.ProcessingInstruction),
    (J.Text = r.Text),
    (J.XMLSerializer = r.XMLSerializer));
  var u = lu();
  return (
    (J.DOMParser = u.DOMParser),
    (J.normalizeLineEndings = u.normalizeLineEndings),
    (J.onErrorStopParsing = u.onErrorStopParsing),
    (J.onWarningStopParsing = u.onWarningStopParsing),
    J
  );
}
fu();
    ...
    vr ||
      ...
      (jt = () => {
        const t = "\\ud800-\\udfff",
          ...
          U = `${v}?`,
          ne = `[${h}]?`,
          he = `(?:${q}(?:${[C, b, y].join("|")})${ne + U})*`,
          E = ne + U + he,
          k = `(?:${[`${C}${m}?`, m, b, y, d, F].join("|")})`;
...
function pu() {
  ...
  var e = t(hu());
  function r(f) {
    if (typeof f != "string") throw new Error("A string is expected as input");
  ...
  Me.length = u;
  function o(f, d, m) {
    if ((d === void 0 && (d = 0), typeof f != "string")) throw new Error("Input must be a string");
  ...
  function a(f, d, m) {
    if ((d === void 0 && (d = 0), typeof f != "string")) throw new Error("Input must be a string");
    var l = u(f);
    if ((typeof d != "number" && (d = parseInt(d, 10)), d >= l)) return "";
    d < 0 && (d += l);
    var v;
    typeof m > "u"
      ? (v = l)
      : (typeof m != "number" && (m = parseInt(m, 10)), (v = m >= 0 ? m + d : d));
  ...
  function p(f, d, m, l) {
    if (
      ...
      m === void 0 && (m = "#"),
      l === void 0 && (l = "right"),
      typeof f != "string" || typeof d != "number")
    )
  ...
  Me.limit = p;
  function h(f, d, m) {
    if ((m === void 0 && (m = 0), typeof f != "string")) throw new Error("Input must be a string");
    if (f === "") return d === "" ? 0 : -1;
    ((m = Number(m)), (m = isNaN(m) ? 0 : m), (d = String(d)));
    var l = r(f);
    if (m >= l.length) return d === "" ? l.length : -1;
    if (d === "") return m;
    var v = r(d),
      C = !1,
      b;
    for (b = m; b < l.length; b += 1) {
      for (var y = 0; y < v.length && v[y] === l[b + y]; ) y += 1;
...
function It(t, e) {
  if (!(e > oe(t) || e < -oe(t))) return Lt(t, e, 1);
}
function ot(t, e) {
  return e < 0 || e > oe(t) - 1 ? "" : Lt(t, e, 1);
}
function du(t, e) {
  if (!(e < 0 || e > oe(t) - 1)) return Lt(t, e, 1).codePointAt(0);
}
function Kr(t, e, r = oe(t)) {
  const u = Zr(t, e);
  return !(u === -1 || u + oe(e) !== r);
}
function mu(t, e, r) {
  if (e < 0) return -1;
  if (r) {
    if (ot(t, e) === "}" && ot(t, e - 1) === "\\") return e;
    const a = Dt(t, "\\}", e);
    return a >= 0 ? a + 1 : a;
  }
  let u = e;
  const o = oe(t);
  for (; u < o && ((u = Dt(t, "}", u)), !(u === -1 || ot(t, u - 1) !== "\\")); ) u += 1;
  return u >= o ? -1 : u;
}
function Wr(t, e) {
  const r = [];
  ...
  function a(h, f, d) {
    const m = Ge(t, o, f),
      l = r.length > 0 && Ie(r[r.length - 1]) ? `${r.pop()}${m}` : m;
  ...
  if (o < p) {
    const h = Ge(t, o);
    r.push(r.length > 0 && Ie(r[r.length - 1]) ? `${r.pop()}${h}` : h);
  }
  return r;
}
function gu(t, e) {
...
function Dt(t, e, r = 0) {
  return st.indexOf(t, e, r);
}
function Zr(t, e, r) {
  let u = r === void 0 ? oe(t) : r;
  u < 0 ? (u = 0) : u >= oe(t) && (u = oe(t) - 1);
  for (let o = u; o >= 0; o--) if (Lt(t, o, oe(e)) === e) return o;
  return -1;
}
function oe(t) {
  return st.length(t);
...
function vu(t, e, r = " ") {
  return e <= oe(t) ? t : st.limit(t, e, r, "right");
}
function Au(t, e, r = " ") {
  return e <= oe(t) ? t : st.limit(t, e, r, "left");
}
function br(t, e) {
...
function zt(t, e, r) {
  const u = oe(t);
  if (e > u || (r && ((e > r && !(e >= 0 && e < u && r < 0 && r > -u)) || r < -u))) return "";
  const o = br(u, e),
    a = r ? br(u, r) : void 0;
  return Ge(t, o, a);
}
function Gt(t, e, r) {
  const u = [];
  if (r !== void 0 && r <= 0) return [t];
  if (e === "") return en(t).slice(0, r);
  let o = e;
  (typeof e == "string" || (e instanceof RegExp && !Qr(e.flags, "g"))) && (o = new RegExp(e, "g"));
  const a = t.match(o);
  let p = 0;
  if (!a) return [t];
  for (let h = 0; h < (r ? r - 1 : a.length); h++) {
    const f = Dt(t, a[h], p),
      d = oe(a[h]);
    if ((u.push(Ge(t, p, f)), (p = f + d), r !== void 0 && u.length === r)) break;
  }
  return (u.push(Ge(t, p)), u);
}
function tr(t, e, r = 0) {
  return Dt(t, e, r) === r;
}
function Lt(t, e = 0, r = oe(t) - e) {
  return st.substr(t, e, r);
}
function Ge(t, e, r = oe(t)) {
  return st.substring(t, e, r);
}
function en(t) {
  return st.toArray(t);
}
...
function yu(t) {
  return t
    ? er(t).map((u) => (Array.isArray(u) ? u.map((o) => new RegExp(o)) : new RegExp(u)))
    : [];
}
function Cu(t) {
  return t ? er(t).map((u) => new RegExp(u)) : [];
}
const Tu =
  /^[\u000C\u000A\u000D\u0009\u000B\u0020\u00a0\u1680\u2000-\u200a\u2028\u2029\u202f\u205f\u3000\u0085]+$/;
function it(t) {
...
function wu(t) {
  ...
  for (let r = 0; r < t.length; r++) {
    ...
    if (u === u.toUpperCase() && u !== u.toLowerCase()) {
      if (r > 0) {
        const a = t[r - 1];
        if (!(a === a.toUpperCase() && a !== a.toLowerCase())) e += "-";
...
const Ht = ["chapter", "book", "para", "row", "sidebar", Vt],
  Su = "​",
  tn = [
  ...
  un = 1,
  on = 1,
  Ou = { book: "GEN", chapterNum: 1, verseNum: 1 },
  an = (t) => {
    var e;
    return ((e = tn[t]) == null ? void 0 : e.chapters) ?? -1;
  },
  Iu = (t, e) => ({
    book: fe.bookNumberToId(Math.max(rn, Math.min(fe.bookIdToNumber(t.book) + e, nn))),
    chapterNum: 1,
    verseNum: 1,
  }),
  xu = (t, e) => ({
    ...t,
    chapterNum: Math.min(Math.max(un, t.chapterNum + e), an(fe.bookIdToNumber(t.book))),
    verseNum: 1,
  }),
  Mu = (t, e) => ({ ...t, verseNum: Math.max(on, t.verseNum + e) });
async function Ru(t, e, r) {
  const u = fe.bookNumberToId(t);
  if (!tr(Intl.getCanonicalLocales(e)[0], "zh"))
    return r({ localizeKey: `LocalizedId.${u}`, languagesToSearch: [e] });
  const o = await r({ localizeKey: `Book.${u}`, languagesToSearch: [e] }),
    a = Gt(o, "-");
  return Gt(a[0], "ÿ08")[0].trim();
}
function Bu(t) {
  return new Jr(fe.bookIdToNumber(t.book), t.chapterNum, t.verseNum).BBBCCC;
}
function Jt(t) {
  return new Jr(fe.bookIdToNumber(t.book), t.chapterNum, t.verseNum).BBBCCCVVV;
}
function _u(t, e) {
  return Jt(t) - Jt(e);
}
...
function Lu(t, e, r, u) {
  ...
  switch (e ?? "id") {
    case "English":
      o = fe.bookIdToEnglishName(t.book);
      break;
    case "id":
      o = t.book;
      break;
...
var cn = ((t) => ((t.OT = "OT"), (t.NT = "NT"), (t.DC = "DC"), (t.Extra = "Extra"), t))(cn || {});
const ku = (t) => {
    if (fe.isBookOT(t)) return "OT";
    if (fe.isBookNT(t)) return "NT";
    if (fe.isBookDC(t)) return "DC";
    if (fe.isExtraMaterial(t)) return "Extra";
...
function Xt(t) {
  ...
  for (let o = 0; o < t.length; o += 1) {
    ...
    (a.charCodeAt(0) < 32
      ? (r || (e += " "), (r = !0))
      : (!r && a === Su && o + 1 < t.length && Nr(t[o + 1])) ||
        (Nr(a) ? (r || (e += a), (r = !0)) : ($u(a) && u === a) || ((e += a), (r = !1))),
      (u = a));
  }
  return e;
}
...
function Cr(t, e) {
  if (!e || !Ht.includes(e.type)) return !1;
  if (!e.content)
    throw new Error(
...
function ln(t, e, r, u) {
  ...
  if (o && a) {
    const p = Xt(t),
      h = Xt(r);
    if (p !== h) {
      if ((!it(It(p, -1) ?? "") && !it(It(h, -1) ?? "")) || !Cr(t, e) || !Cr(r, u)) return !1;
      let f = p;
      for (; it(It(f, -1) ?? ""); ) f = zt(f, 0, -1);
      let d = h;
      for (; it(It(d, -1) ?? ""); ) d = zt(d, 0, -1);
      if (f !== d) return !1;
    }
  } else if (!o && !a) {
    const p = t,
      h = r,
      f = Object.keys(p).filter((l) => l !== "content");
    if (
      f.length !== Object.keys(h).filter((l) => l !== "content").length ||
      f.some((l) => !(l in h) || p[l] !== h[l])
    )
      return !1;
    const d = yr(p.content),
      m = yr(h.content);
    if (d !== m) return !1;
    if (!d && !m) {
      let l = p.content,
      ...
      Ht.includes(p.type) && Ie(C) && it(C) && (l = l.slice(0, -1));
      const b = v[v.length - 1];
      if ((Ht.includes(h.type) && Ie(b) && it(b) && (v = v.slice(0, -1)), l.length !== v.length))
        return !1;
      for (let y = 0; y < l.length; y += 1) if (!ln(l[y], p, v[y], h)) return !1;
    }
  } else return !1;
  return !0;
}
function ju(t, e) {
  return ln(t, void 0, e, void 0);
}
const Uu =
    (t) =>
    (...e) =>
      t.map((u) => u(...e)).every((u) => u),
  Vu =
    (t) =>
    async (...e) => {
...
function _t(t) {
  return function (r, u, o) {
    if (!r || !u || typeof r != "object" || typeof u != "object") return t(r, u, o);
    var a = o.cache,
      p = a.get(r),
      h = a.get(u);
    if (p && h) return p === u && h === r;
    (a.set(r, u), a.set(u, r));
    var f = t(r, u, o);
...
function Wu(t, e, r) {
  ...
  for (; u-- > 0; ) if (!r.equals(t[u], e[u], u, u, t, e, r)) return !1;
  return !0;
}
function Qu(t, e) {
  return ct(t.getTime(), e.getTime());
}
function Zu(t, e) {
...
function ri(t, e, r) {
  var u = Or(t),
    o = u.length;
  if (Or(e).length !== o) return !1;
  for (; o-- > 0; ) if (!fn(t, e, r, u[o])) return !1;
  return !0;
}
function St(t, e, r) {
  var u = wr(t),
    o = u.length;
  if (wr(e).length !== o) return !1;
  for (var a, p, h; o-- > 0; )
    if (
      ((a = u[o]),
      !fn(t, e, r, a) ||
        ...
        (p || h) &&
          (!p ||
            !h ||
            p.configurable !== h.configurable ||
            p.enumerable !== h.enumerable ||
            p.writable !== h.writable)))
    )
      return !1;
  return !0;
}
function ni(t, e) {
  return ct(t.valueOf(), e.valueOf());
}
function ui(t, e) {
...
function fn(t, e, r, u) {
  return (u === Ku || u === Yu || u === Xu) && (t.$$typeof || e.$$typeof)
    ? !0
    : Ju(e, u) && r.equals(t[u], e[u], u, u, t, e, r);
}
var ai = "[object Arguments]",
  ...
  Ei = "[object URL]",
  Di = Array.isArray,
  Mr = typeof ArrayBuffer == "function" && ArrayBuffer.isView ? ArrayBuffer.isView : null,
  Rr = Object.assign,
  vi = Object.prototype.toString.call.bind(Object.prototype.toString);
function Ai(t) {
  ...
  return function (b, y, q) {
    if (b === y) return !0;
    if (b == null || y == null) return !1;
    var R = typeof b;
    if (R !== typeof y) return !1;
    if (R !== "object") return R === "number" ? p(b, y, q) : R === "function" ? o(b, y, q) : !1;
    var F = b.constructor;
    if (F !== y.constructor) return !1;
    if (F === Object) return h(b, y, q);
    if (Di(b)) return e(b, y, q);
    if (Mr != null && Mr(b)) return l(b, y, q);
    if (F === Date) return r(b, y, q);
    if (F === RegExp) return d(b, y, q);
    if (F === Map) return a(b, y, q);
    if (F === Set) return m(b, y, q);
    var U = vi(b);
    return U === ci
      ? r(b, y, q)
      : U === di
        ...
        : U === fi
          ...
          : U === mi
            ? m(b, y, q)
            : U === pi
              ? typeof b.then != "function" && typeof y.then != "function" && h(b, y, q)
              : U === Ei
                ? v(b, y, q)
                : U === li
                  ? u(b, y, q)
...
function bi(t) {
  var e = t.circular,
    ...
    u = t.strict,
    o = {
      areArraysEqual: u ? St : Wu,
      areDatesEqual: Qu,
      areErrorsEqual: Zu,
      areFunctionsEqual: ei,
      areMapsEqual: u ? Tr(Ir, St) : Ir,
      areNumbersEqual: ti,
      areObjectsEqual: u ? St : ri,
      arePrimitiveWrappersEqual: ni,
      areRegExpsEqual: ui,
      areSetsEqual: u ? Tr(xr, St) : xr,
      areTypedArraysEqual: u ? St : ii,
      areUrlsEqual: oi,
    };
  if ((r && (o = Rr({}, o, r(o))), e)) {
    var a = _t(o.areArraysEqual),
      p = _t(o.areMapsEqual),
      h = _t(o.areObjectsEqual),
      f = _t(o.areSetsEqual);
    o = Rr({}, o, { areArraysEqual: a, areMapsEqual: p, areObjectsEqual: h, areSetsEqual: f });
  }
  return o;
}
function Ni(t) {
...
function yi(t) {
  ...
  if (u)
    return function (f, d) {
      var m = u(),
        l = m.cache,
        v = l === void 0 ? (e ? new WeakMap() : void 0) : l,
        C = m.meta;
      return r(f, d, { cache: v, equals: o, meta: C, strict: a });
    };
  if (e)
    return function (f, d) {
      return r(f, d, { cache: new WeakMap(), equals: o, meta: void 0, strict: a });
    };
  var p = { cache: void 0, equals: o, meta: void 0, strict: a };
  return function (f, d) {
    return r(f, d, p);
  };
}
...
function He(t) {
  ...
  var e = t.circular,
    ...
    f = Ai(h),
    d = u ? u(f) : Ni(f);
  return yi({ circular: r, comparator: f, createState: o, equals: d, strict: p });
}
function hn(t, e) {
  return Ci(t, e);
}
function pn(t, e) {
  ...
  if (typeof t != "object") return hn(t, e);
  const r = e,
    u = t;
  let o = !0;
  return (
    Object.keys(r).forEach((a) => {
      o && ((Object.hasOwn(u, a) && pn(u[a], r[a])) || (o = !1));
    }),
    o
  );
}
...
function dn(t, e) {
  ...
  if (u !== null) return typeof u == "object" ? r(u) : u;
}
function Ti(t) {
  try {
    const e = Yt(t);
...
function Si() {
  return typeof navigator < "u" && navigator.languages
    ? navigator.languages[0].replace(/@posix$/i, "")
    : new Br().resolvedOptions().locale;
}
function Oi(t, e = 2) {
  if (t === 0) return "0 Bytes";
  const r = ["Bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"],
    u = Math.floor(Math.log(t) / Math.log(1024)),
    o = r[u];
  return `${new qr("en", { style: "decimal", maximumFractionDigits: e, minimumFractionDigits: 0 }).format(t / 1024 ** u)} ${o}`;
}
const Ii = 1e3,
  mn = 60,
  gn = mn * 60,
  xi = gn * 24;
function Mi(t, e, r = new Date()) {
  const u = Math.floor((e.getTime() - r.getTime()) / Ii),
    o = Math.round(u / xi);
  if (Math.abs(o) >= 1) return t.format(o, "day");
  const a = Math.round(u / gn);
  ...
  rr = {
    projectSettingsContribution: {
      ...
      anyOf: [
        ...
        { type: "array", items: { $ref: "#/$defs/projectSettingsGroup" } },
      ],
    },
    projectSettingsGroup: {
      ...
      properties: {
        ...
        properties: { $ref: "#/$defs/projectSettingProperties" },
      },
    ...
    projectSettingProperties: {
      ...
      type: "object",
      patternProperties: { "^[\\w\\-]+\\.[\\w\\-]+$": { $ref: "#/$defs/projectSetting" } },
      additionalProperties: !1,
    },
    projectSetting: {
      description: "A description of an extension's setting entry",
      anyOf: [{ $ref: "#/$defs/extensionControlledProjectSetting" }],
    },
    ...
    projectSettingBase: {
      ...
      allOf: [{ $ref: "#/$defs/settingBase" }, { $ref: "#/$defs/modifierProject" }],
    },
    modifierProject: {
    ...
    settingsContribution: {
      ...
      anyOf: [
        ...
        { type: "array", items: { $ref: "#/$defs/settingsGroup" } },
      ],
    },
    settingsGroup: {
      description: "Group of related settings definitions",
      ...
      properties: {
        ...
        properties: { $ref: "#/$defs/settingProperties" },
      },
      required: ["label", "properties"],
    },
    settingProperties: {
      description: "Object whose keys are setting IDs and whose values are settings objects",
      type: "object",
      patternProperties: { "^[\\w-]+\\.[\\w-]+$": { $ref: "#/$defs/setting" } },
      additionalProperties: !1,
    },
    setting: {
      description: "A description of an extension's setting entry",
      anyOf: [{ $ref: "#/$defs/extensionControlledSetting" }],
    },
    extensionControlledSetting: {
      description: "Setting definition that is validated by the extension.",
      allOf: [{ $ref: "#/$defs/settingBase" }, { $ref: "#/$defs/modifierExtensionControlled" }],
    },
    settingBase: {
    ...
    userStateProperties: {
      ...
      type: "object",
      patternProperties: { "^[\\w\\-]+\\.[\\w\\-]+$": { $ref: "#/$defs/userState" } },
      additionalProperties: !1,
    },
    userState: {
      description: "A description of an extension's user state entry",
      anyOf: [{ $ref: "#/$defs/extensionControlledState" }],
    },
    extensionControlledState: {
      description: "State definition that is validated by the extension.",
      allOf: [{ $ref: "#/$defs/stateBase" }, { $ref: "#/$defs/modifierExtensionControlled" }],
    },
    modifierExtensionControlled: {
      ...
      not: {
        anyOf: [
          { type: "object", required: ["platformType"] },
    ...
    stateBase: {
      ...
      type: "object",
      properties: {
        default: { description: "default value for the state/setting", type: "any" },
        derivesFrom: {
          description:
    ...
    id: { description: "", type: "string", pattern: "^[\\w\\-]+\\.[\\w\\-]+$", tsType: "Id" },
  };
function kt(t) {
  t &&
...
const vn = {
  languageStrings: {
    ...
    type: "object",
    patternProperties: { "^%[\\w\\-\\.]+%$": { $ref: "#/$defs/localizedStringValue" } },
    additionalProperties: !1,
  },
  ...
  stringsMetadata: {
    ...
    type: "object",
    patternProperties: { "^%[\\w\\-\\.]+%$": { $ref: "#/$defs/stringMetadata" } },
    additionalProperties: !1,
  },
  stringMetadata: {
...
const An = {
  ...
  type: "object",
  properties: {
    metadata: { $ref: "#/$defs/stringsMetadata" },
    localizedStrings: { type: "object", additionalProperties: { $ref: "#/$defs/languageStrings" } },
  },
  $defs: vn,
};
Object.freeze(An);
const bn = {
  ...
  properties: {
    ...
    webViewMenus: {
      ...
      type: "object",
      patternProperties: { "^[\\w\\-]+\\.[\\w\\-]+$": { $ref: "#/$defs/menusForOneWebView" } },
      additionalProperties: !1,
    },
  },
  ...
  $defs: {
    ...
    groupsAndItems: {
      ...
      type: "object",
      properties: {
        groups: { description: "Groups that belong in this menu", $ref: "#/$defs/menuGroups" },
        items: {
...
const Nn = {
  ...
  themeDefinition: {
    ...
    properties: {
      ...
      cssVariables: { $ref: "#/$defs/themeCssVariables" },
    },
...
A theme type indicates the kind of theme (e.g. light, dark). Some UI elements use the theme type to determine how to look. Colors not present in the theme will fall back to the built-in colors for this type.`,
    type: "object",
    properties: {
      light: { $ref: "#/$defs/themeDefinition" },
      dark: { $ref: "#/$defs/themeDefinition" },
    },
    additionalProperties: { anyOf: [{ $ref: "#/$defs/themeDefinition" }, { type: "null" }] },
  },
  themeFamiliesById: {
    description: "Object whose keys are theme family ids and whose values are {@link ThemeFamily}.",
    type: "object",
    additionalProperties: { anyOf: [{ $ref: "#/$defs/themeFamily" }, { type: "null" }] },
  },
};
kt(Nn);
const yn = {
  ...
  anyOf: [{ $ref: "#/$defs/themeFamiliesById" }],
  $defs: Nn,
};
Object.freeze(yn);
const Cn = "theme-styles";
function Bi(t, e) {
  return `${t ? `${t}-` : ""}${e}`;
}
function _i(t, e) {
...
function Pi(t, e, r) {
  ...
  (u && this.document.body.classList.remove(u),
    this.document.body.classList.add(t.id),
    e && this.document.head.removeChild(e));
  const o = this.document.createElement("style");
  return (
    (o.id = `${Cn}${r ? `-${r}` : ""}`),
    (o.dataset.themeId = t.id),
    (o.textContent = Tn(t)),
...
class ie {
  ...
  findSingleValue(e) {
    const r = or.JSONPath({ path: e, json: this.usj, wrap: !0 });
    if (r === void 0 || r.length === 0) return;
    if (!Array.isArray(r[0])) return r[0];
    const u = or.JSONPath({ path: e, json: this.usj, wrap: !1 });
  ...
  createUsjParentMap() {
    const e = new Map();
    return (
      this.usj.content && e.set(this.usj.content, this.usj),
      this.usj.content.forEach((r) => {
  ...
  get parentMap() {
    return this.parentMapInternal
      ...
      : ((this.parentMapInternal = this.createUsjParentMap()), this.parentMapInternal);
  }
  ...
  createWorkingStack(e) {
    ...
    let o = e,
      a = u.get(o);
    for (; a !== void 0; ) {
      if (!a.content) throw new Error("Invalid parentMap: all parents should have content");
      if (
        !a.content.find((p, h) => {
          if (p !== o) return !1;
  ...
  static findRightMostDescendantMarkerObject(e, r, u = []) {
    ...
    for (let o = e.content.length - 1; o >= 0; o--) {
      ...
      if (typeof a == "object" && !u.includes(a.type))
        return a.content
          ? this.findRightMostDescendantMarkerObject(a, e, u)
          : { node: a, parent: e };
    }
  ...
  findNextMatchingNode(e, r, u) {
    ...
    return ie.findNextMatchingNodeUsingWorkingStack(e, o, r, u);
  }
  nodeToJsonPath(e) {
  ...
  findVerseRefForNode(
    e,
    r,
    u = { chapterNum: void 0, verseNum: void 0, startingContentNode: void 0 },
  ) {
    ...
    if (typeof e == "object" && e.number !== void 0) {
      ...
      if (e.type === xt)
        return (
          (u.chapterNum = f),
          (u.verseNum = u.verseNum ?? 0),
          (u.startingContentNode = u.startingContentNode ?? e),
          u
        );
      e.type === Mt && !u.verseNum && ((u.verseNum = f), (u.startingContentNode = e));
    }
    if (!r.content) throw new Error(`"content" array not found: ${JSON.stringify(r)}`);
    let o = 0;
    ...
    let a = o - 1;
    for (; a >= 0 && typeof r.content[a] != "object"; ) a -= 1;
    if (a < 0) {
      if (r.type === Vt)
        return (
          u.chapterNum === void 0 &&
            ((u.chapterNum = 1), (u.verseNum = 0), (u.startingContentNode = void 0)),
          u
        );
      const f = r,
        d = this.parentMap.get(f);
      if (!d) throw new Error(`No parent found for ${JSON.stringify(r)}`);
      return this.findVerseRefForNode(f, d, u);
    }
    const p = r.content[a],
      h = ie.findRightMostDescendantMarkerObject(p, r, je);
  ...
  nodeToVerseRefAndOffset(e, r, u) {
    ...
    if (
      (u === void 0
        ? (o = this.parentMap.get(r))
        : (o = Array.isArray(u) ? this.parentMap.get(u) : u),
      o === void 0)
    )
    ...
    const p = { book: e, chapterNum: a.chapterNum, verseNum: a.verseNum ?? 0 };
    let h = 0;
    return (
      a.startingContentNode !== void 0 &&
        this.findNextMatchingNode(a.startingContentNode, [], (f, d) => {
          var m, l;
          return f === r
            ? !0
            : d.find((v) => je.includes(v.parent.type))
              ? !1
              : typeof f == "string"
                ? ((h += f.length), !1)
                : (f.type === xt &&
                      f.number !== ((m = a.chapterNum) == null ? void 0 : m.toString())) ||
                    (f.type === Mt &&
                      f.number !== ((l = a.verseNum) == null ? void 0 : l.toString()))
                  ? ((h = 0), !0)
                  : !1;
        }),
      { verseRef: p, offset: h }
  ...
  jsonPathToVerseRefAndOffset(e, r) {
    ...
    if (!p) throw new Error(`Could not determine SerializedVerseRef that corresponds to ${e}`);
    return p;
  }
  verseRefToUsjContentLocation(e, r = 0) {
    ...
    if (o === void 0) throw new Error(`Could not find ${u} chapter ${e.chapterNum}`);
    let a = !1,
      p = "";
    const h = e.verse ?? e.verseNum.toString(),
      f = this.findNextMatchingNode(o, je, (v, C) =>
        v === o
          ? e.verseNum === 0
            ? ((p = ie.convertWorkingStackToJsonPath(C)), !0)
            : !1
          : typeof v != "object"
            ? !1
            : v.type === xt
              ? ((a = !0), !0)
              : v.type === Mt && v.number !== void 0 && v.number === h
                ? ((p = ie.convertWorkingStackToJsonPath(C)), !0)
                : !1,
      );
    ...
    let d = 0,
      ...
      l;
    return (
      this.findNextMatchingNode(f, je, (v, C) => {
        if (v === f) return !1;
        if (typeof v == "string") {
          if (((d += v.length), d > r))
            return ((p = ie.convertWorkingStackToJsonPath(C)), (m = r - d + v.length), (l = v), !0);
        } else if (v.type === xt || v.type === Mt) return !0;
        return !1;
      }),
  ...
  verseRefToNextTextLocation(e) {
    const r = this.verseRefToUsjContentLocation(e),
      u = this.findNextLocationOfMatchingText(r, "");
    if (!u)
      throw new Error(
  ...
  findNextLocationOfMatchingText(e, r, u = 1e3) {
    let o = "",
      ...
      p = 0,
      h = -1;
    const f = this.convertJsonPathToWorkingStack(e.jsonPath),
      d = { ...f[f.length - 1] };
    if (
      (ie.findNextMatchingNodeUsingWorkingStack(e.node, f, je, (C, b) => {
        if (typeof C != "string") return !1;
        let y = C;
        const q = b[b.length - 1];
        (ie.areStackItemsShallowEqual(q, d) && ((y = C.substring(e.offset)), (p += e.offset)),
          (a += y.length),
          (o = `${o}${y}`));
        const R = o.indexOf(r);
        return R < 0
          ? ((p += o.length),
            o.length > r.length && (o = o.substring(o.length - r.length)),
            (p -= o.length),
            a > u)
          : ((h = p + R), !0);
      }),
    ...
    const v = ie.findNextMatchingNodeUsingWorkingStack(
      ...
      je,
      (C, b) =>
        typeof C != "string" || ((a += C.length), a < h + 1)
          ? !1
          : ((m = h - a + C.length), (l = b), !0),
    );
    if (!v) throw new Error("Internal error: inconsistent search results");
    return { node: v, offset: m, jsonPath: ie.convertWorkingStackToJsonPath(l) };
  }
  search(e) {
    const r = [];
    if (this.usj.content.length === 0) return r;
    const u = { node: this.usj.content[0], jsonPath: "$.content[0]" },
      o = [],
      a = new $r();
    let p = 0,
      h = u.node;
    for (; h !== void 0; )
      h = ie.findNextMatchingNodeUsingWorkingStack(
        ...
        (m, l) => (
          typeof m != "string" ||
            (o.push(m),
            a.set(p, { node: m, offset: 0, jsonPath: ie.convertWorkingStackToJsonPath(l) }),
            (p += m.length)),
          !1
        ),
      );
    ...
    for (; d; ) {
      if (d[0].length > 0) {
        ...
        if (!m) throw new Error(`Internal error: no closest node found for index ${d.index}`);
        const l = { node: m.value.node, offset: d.index - m.key, jsonPath: m.value.jsonPath };
  ...
  extractText(e, r) {
    let u = "",
      ...
      a = 0;
    return (
      ie.findNextMatchingNodeUsingWorkingStack(
        e.node,
        this.convertJsonPathToWorkingStack(e.jsonPath),
        je,
        (p) => {
          if (typeof p != "string") return !1;
          if (o >= p.length) return ((o -= p.length), !1);
          let h = p;
  ...
  extractTextBetweenPoints(e, r, u = 100) {
    ...
    return (
      ie.findNextMatchingNodeUsingWorkingStack(
        ...
        (a, p) =>
          a === r.node &&
          (typeof a == "object" || r.jsonPath === ie.convertWorkingStackToJsonPath(p))
            ? !0
            : typeof a != "string"
              ? !1
              : ((o = `${o}${a}`), o.length > u && (o = o.substring(0, u)), o.length >= u),
      ),
      o
    );
  }
  static removeContentNodesFromArray(e, r) {
    ...
    for (let o = e.length - 1; o >= 0; o--) {
      ...
      r(a)
        ...
        : typeof a != "string" &&
          a.content &&
          (u += this.removeContentNodesFromArray(a.content, r));
    }
    return u;
  }
...
exports.AsyncVariable = Ut;
exports.CHAPTER_TYPE = xt;
exports.Collator = In;
exports.DateTimeFormat = Br;
exports.DocumentCombiner = Lr;
exports.EventRollingTimeCounter = jn;
exports.FIRST_SCR_BOOK_NUM = rn;
exports.FIRST_SCR_CHAPTER_NUM = un;
exports.FIRST_SCR_VERSE_NUM = on;
exports.LAST_SCR_BOOK_NUM = nn;
exports.MODIFIER_KEYS = Ri;
exports.Mutex = Fr;
exports.MutexMap = Un;
exports.NonValidatingDocumentCombiner = Vn;
exports.NumberFormat = qr;
exports.PLATFORM_ERROR_VERSION = Ot;
exports.PlatformEventEmitter = _r;
exports.PromiseChainingMap = Gn;
exports.Section = cn;
exports.SortedNumberMap = $r;
exports.SortedSet = Hn;
exports.THEME_STYLE_ELEMENT_ID = Cn;
exports.UnsubscriberAsyncList = Jn;
exports.UsjReaderWriter = ie;
exports.VERSE_TYPE = Mt;
exports.aggregateUnsubscriberAsyncs = Vu;
exports.aggregateUnsubscribers = Uu;
exports.applyThemeStylesheet = Pi;
exports.areUsjContentsEqualExceptWhitespace = ju;
exports.at = It;
exports.charAt = ot;
exports.codePointAt = du;
exports.compareScrRefs = _u;
exports.createSyncProxyForAsyncObject = kn;
exports.debounce = Mn;
exports.deepClone = Et;
exports.deepEqual = hn;
exports.defaultScrRef = Ou;
exports.deserialize = dn;
exports.endsWith = Kr;
exports.ensureArray = er;
exports.escapeStringRegexp = Nu;
exports.expandThemeContribution = _i;
exports.formatBytes = Oi;
exports.formatReplacementString = gu;
exports.formatReplacementStringToArray = Wr;
exports.formatScrRef = Lu;
exports.formatTimeSpan = Mi;
exports.getAllObjectFunctionNames = Ln;
exports.getChaptersForBook = an;
exports.getCurrentLocale = Si;
exports.getErrorMessage = Kt;
exports.getLocalizeKeyForScrollGroupId = sn;
exports.getLocalizeKeysForScrollGroupIds = Pu;
exports.getLocalizedIdFromBookNumber = Ru;
exports.getSectionForBook = ku;
exports.getStylesheetForTheme = Tn;
exports.groupBy = Rn;
exports.htmlEncode = wi;
exports.includes = Qr;
exports.indexOf = Dt;
exports.isErrorMessageAboutParatextBlockingInternetAccess = Fn;
exports.isErrorMessageAboutRegistryAuthFailure = qn;
exports.isLocalizeKey = bu;
exports.isPlatformError = Yn;
exports.isSerializable = Ti;
exports.isString = Ie;
exports.isSubset = pn;
exports.isWhiteSpace = it;
exports.lastIndexOf = Zr;
exports.localizedStringsDocumentSchema = An;
exports.menuDocumentSchema = bn;
exports.newGuid = xn;
exports.newPlatformError = Xn;
exports.normalize = Eu;
exports.normalizeScriptureSpaces = Xt;
exports.offsetBook = Iu;
exports.offsetChapter = xu;
exports.offsetVerse = Mu;
exports.ordinalCompare = Du;
exports.padEnd = vu;
exports.padStart = Au;
exports.projectSettingsDocumentSchema = En;
exports.scrRefToBBBCCC = Bu;
exports.scrRefToBBBCCCVVV = Jt;
exports.serialize = Yt;
exports.settingsDocumentSchema = Dn;
exports.slice = zt;
exports.split = Gt;
exports.startsWith = tr;
exports.stringLength = oe;
exports.substring = Ge;
exports.themeDocumentSchema = yn;
exports.toArray = en;
exports.toKebabCase = wu;
exports.transformAndEnsureRegExpArray = Cu;
exports.transformAndEnsureRegExpRegExpArray = yu;
exports.wait = Pr;
exports.waitForDuration = Pn;
