...
import { MarginalRef } from "@eten-tech-foundation/platform-editor";
import { RefObject } from "react";

interface ExternalToolbarProps {
  marginalRef: RefObject<MarginalRef | null>;
}

export default function ExternalToolbar({ marginalRef }: ExternalToolbarProps) {
  const handleInsertFootnote = () => {
    marginalRef.current?.insertNote("f");
  };

  const handleInsertCrossReference = () => {
    marginalRef.current?.insertNote("x");
  };

  const handleInsertEndnote = () => {
    marginalRef.current?.insertNote("fe");
  };
